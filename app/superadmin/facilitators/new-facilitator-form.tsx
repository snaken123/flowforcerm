...
export function NewFacilitatorForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const res = await fetch("/api/superadmin/facilitators", {
        method: "POST",
      ...
      if (!res.ok) {
        setError(body.error ?? "Failed to create facilitator.");
        return;
      }
      setName("");
      setEmail("");
    ...
    <form onSubmit={onSubmit} className="rounded-xl border border-white/10 bg-[#111] p-6 mb-6 grid grid-cols-3 gap-4">
      ...
      <div className="col-span-3 flex gap-3 justify-end">
        ...
        >
          ...
          {submitting ? "Creating…" : "Create Facilitator"}
        </button>
      </div>
    </form>
  );
